...
| Tally     | `C:\src\tally`        | Net worth, sales evaluation, ledger snapshots   | In development (pre-release)    |
...
Tally was previously sketched as a planned cog called "Ledger" — that name is retired. Tally IS the ledger; if you see "Ledger" anywhere in older notes, read it as Tally.
...
3. **`C:\src\cogworks\docs\PLAN.md`** — full integration plan: architecture decision, Syndicator strategy, phased rollout (Phases 0-7), Tally sketch, branding voice, risk table (~4 min)

If you're going to touch inventory code (FlipQueue or Tally):
...
- **Touching inventory data?** Read `SYNDICATOR_INTEGRATION.md` first. Inventory-aware cogs (FlipQueue, Tally) read from Syndicator, not from their own scanners.
