...
-- The Theme and Settings pages are both thin demos of the one Appearance
-- editor primitive, cw:CreateUIScalingSettingsBlock. Each passes a different
-- opts flag so the showcase doubles as documentation for those flags:
--   Theme    -> { showFontFamily = false }  (theme + colors, fonts hidden)
--   Settings -> { showTheme = false }       (fonts + scale, theme hidden)
-- The Appearance tab page demos the unflagged, full editor inside a TabPanel.

pages.theme = function(parent)
  local f = CreateFrame("Frame", nil, parent)
  f:SetAllPoints()

  cw:CreateUIScalingSettingsBlock(f, {
    showFontFamily = false,
    description    = "Theme & colors, straight from the cw:CreateUIScalingSettingsBlock "
                  .. "primitive with { showFontFamily = false } -- the font-family section "
                  .. "is switched off via opts to keep the focus on theming. Pick a preset "
                  .. "or custom theme from the dropdown, click any swatch to recolor it, and "
                  .. "save / export / import the result. Every edit repaints the live "
                  .. "preview above. The same editor (unflagged) backs the Appearance tab.",
  })
  return f
end
...
pages.settings = function(parent)
  ...
  cw:CreateUIScalingSettingsBlock(f, {
    showTheme   = false,
    description = "Fonts & scale, straight from the cw:CreateUIScalingSettingsBlock "
               .. "primitive with { showTheme = false } -- the theme & colors section "
               .. "is switched off via opts to keep the focus on typography. Adjust the "
               .. "font family, font scale and UI scale; the live preview above repaints "
               .. "as you go. The same editor (unflagged) backs the Appearance tab.",
  })
  return f
end
