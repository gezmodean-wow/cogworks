...
- **`CreateSegmentedControl` per-option `disabled` + `count` badges** (`SegmentedControl.lua`, MODULE_MINOR `1 → 2`, COG-39) — options now accept `count` (a brass-tinted badge after the label) and `disabled` (dim + unclickable + hover suppressed). New `seg:SetOptionState(key, { count = .., disabled = .. })` mutates a single segment without rebuilding the strip (`count = false` clears the badge; a `nil` field is left unchanged). A disabled option never auto-pops selection elsewhere — callers move selection themselves.
- **`CreateDropdown` `opts.itemBuilder` custom rows** (`Cogworks-1.0.lua`, COG-45) — optional `itemBuilder(rowFrame, item, isSelected)` renders multi-line / sublabel / badge rows. The lib still owns selection paint + the row click handler; builders use non-mouse regions so clicks fall through. Rebuilt only when a row's bound item changes. Omitting it preserves flat-label rendering exactly. Set `item.width` so auto-width can measure custom rows.
- **`CreateThemedMainFrame` scrollable sidebar** (`ThemedMainFrame.lua`, MODULE_MINOR `2 → 3`, COG-37) — `sidebar.scrollable = true` puts the nav stack in an internal `ScrollFrame` that grows with the item count; the mouse wheel scrolls it once nav overflows the sidebar height, and `SetActivePage` auto-scrolls the active item into view. Defaults `false` (nav anchors straight to the sidebar, unchanged). The drag handle still resizes width.
- **Appearance tab `opts.extensions` hook** (`Scaling.lua`, MODULE_MINOR `5 → 6`, COG-73) — `cw:CreateAppearanceTab(parent, { extensions = function(block, yOffset) ... return consumedHeight end })` appends a few cog-specific appearance rows below the standard editor without splitting the cog's appearance UI across tabs. The Reset button stays at the very bottom. (Completes COG-73; the in-game theme color editor shipped in v0.15.0.)
- **Toast drag-to-move** (`Toast.lua`, MODULE_MINOR `1 → 2`, COG-79) — **player-facing.** A draggable on-screen ghost lets players reposition toasts: it snaps to the nearest screen corner on release and persists through `toastAnchor`, so live toasts and the next `/reload` respect it. New API `cw:EnterToastMoveMode()` / `ExitToastMoveMode()` / `IsToastMoveMode()` / `ResetToastAnchor()`. The standard entry point is a **"Move toasts on screen"** button in the suite Appearance tab (gated by `opts.showToasts`, default on), so every cog that adopts the Appearance tab gets it for free.
- **`CreateAnchoredPopup` — anchored, non-modal action popup** (`AnchoredPopup.lua`, new module, MODULE_MINOR `1`, COG-50) — the BankPopup shape none of the existing primitives fit: pins next to an anchor frame (`above` / `below` / `left` / `right`) with no modal dim, an optional progress strip (frame-rate-independent exponential smoothing on the main bar + an optional self-counting heartbeat sub-bar for inter-step waits), and an optional single-button footer band. `SetProgress` / `SetProgressText` / `Heartbeat` / `SetAnchor` / `Reanchor`; `pop.content` for the body.
- **MiniView inner-row primitives** (`MiniView.lua`, MODULE_MINOR `17 → 18`, COG-51) — composable body to match the COG-41 shell: `mini:AddPartnerStrip({ label, icon })` for multi-character clusters and `mini:AddTaskRow({ icon, text, tooltip, actions, onClick, onRightClick })` (rows added through a strip indent under it). Right-aligned action buttons with per-action `visible(rowData)` gates, tooltip wiring, and left/right click distinction. `mini:ClearRows()` / `mini:GetRowsHeight()`.
- **`CreateScrollTable` column reorder + horizontal scroll + persistable layout + row actions** (`Cogworks-1.0.lua`, COG-46 / COG-52) — the table already shipped column drag-resize and sort; the optional 3rd arg `opts` adds the rest: `allowReorder` (drag headers to reorder), `horizontalScroll` (SHIFT+wheel scrolls when columns overflow the view), `saveLayoutTo` / `onLayoutChange` (persist `{ [colKey] = { width, order } }`, applied on construction and written on every resize/reorder commit), and `rowActions` / `tbl:EnableRowActions(defs)` (hover-revealed right-aligned per-row buttons with `visible(rowData)` gates). Columns honor `col.minWidth` on resize. No-opt callers are unchanged.

### Notes

- **No removals.** Per the additive-only rule, every existing `CreateScrollTable` / `CreateDropdown` / `CreateSegmentedControl` / `CreateThemedMainFrame` / `CreateMiniView` / `CreateAppearanceTab` call behaves exactly as before unless it opts into a new field.
- The reusable `verify-package` CI workflow (COG-27) and the v0.15.0 in-game theme color editor (COG-73 phase 1) already shipped; this release closes those trackers alongside the new work.
- Player-visible surface in the **standalone** addon: the toast move-mode overlay. It reaches players of sibling cogs only as those cogs adopt the Appearance tab.
