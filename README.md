...
- **Scaling settings block** (`Scaling.lua`) — `CreateUIScalingSettingsBlock` — drop-in section for any cog's settings page (profile dropdown, font / UI scale sliders, font family + theme dropdowns, reset, optional per-cog override row); `RegisterScalingFrame` subscribes a frame to `uiScale` changes and persists position / size into caller-owned SVs
