...
- **Segmented control** (`SegmentedControl.lua`) — `CreateSegmentedControl` — horizontal pill-button group with a one-active invariant; right shape for inline state-only filters ("7d / 30d / 90d", "Saleable / Owned"); distinct from `CreateTabPanel` (owns content swap) and `CreateNavButton` (sidebar nav)
- **Appearance editor** (`Scaling.lua`) — `CreateAppearanceTab` / `CreateUIScalingSettingsBlock` — a full scrolling theme/font editor that drops straight into a settings tab: live preview panel, profile row (New / Export / Import), optional per-cog override, font / UI scale sliders, font-family picker with previews, theme dropdown, click-to-edit color swatches, and theme save / export / import. `opts.showFontFamily` / `opts.showTheme` gate sections off. `RegisterScalingFrame` subscribes a frame to `uiScale` changes and persists position / size into caller-owned SVs
...
- **Themed main frame** (`ThemedMainFrame.lua`) — `CreateThemedMainFrame` — one-call main-window chrome (title bar, summary row, sidebar with drag-resize handle, content area, resize grip, ESC-to-close, persisted geometry); `frame:AddNavItem`, `:SetActivePage`, `:SetSummary`, `:SetPageBuilder` cover the common surface
- **Drawer** (`Drawer.lua`) — `CreateDrawer` — non-modal floating panel for tool drawers, context drawers, side popups; title + close + drag, optional resizable + grip, optional anchor-to-parent on Show
...
- **Toast** (`Toast.lua`) — `Toast(opts)` — transient banners with fade in/out, vertical stacking, severity → theme color (`success`/`info`/`warning`/`error`), optional icon + onClick, click-to-dismiss, hover pauses auto-dismiss
